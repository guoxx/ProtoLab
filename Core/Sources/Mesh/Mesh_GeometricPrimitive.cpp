...
void Mesh::loadCoordinateSystemFrame()
{
	...
	xAxis->_positionBuffer = RHI::getInstance().getDevice()->createVertexBuffer(pVexBuf+0, sizeof(Vertex)*2);
	xAxis->_indexBuffer = RHI::getInstance().getDevice()->createIndexBuffer(idxBuf, sizeof(idxBuf));
	...
	yAxis->_positionBuffer = RHI::getInstance().getDevice()->createVertexBuffer(pVexBuf+2, sizeof(Vertex)*2);
	yAxis->_indexBuffer = RHI::getInstance().getDevice()->createIndexBuffer(idxBuf, sizeof(idxBuf));
	...
	zAxis->_positionBuffer = RHI::getInstance().getDevice()->createVertexBuffer(pVexBuf+4, sizeof(Vertex)*2);
	zAxis->_indexBuffer = RHI::getInstance().getDevice()->createIndexBuffer(idxBuf, sizeof(idxBuf));
...
std::shared_ptr<Mesh> Mesh::createSphere(float diameter, size_t tessellation, bool rhcoords, bool invertn)
{
	...
	prim->_positionBuffer = RHI::getInstance().getDevice()->createVertexBuffer(vertices.data(), vertices.size() * sizeof(DirectX::VertexPositionNormalTexture));
	prim->_indexBuffer = RHI::getInstance().getDevice()->createIndexBuffer(indices.data(), indices.size() * sizeof(uint16_t));
	prim->_matIdx = 0;
